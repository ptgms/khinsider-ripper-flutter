...
class _DownloadingViewState extends State<DownloadingView> {
  ...
  Future<bool> downloadAlbum(AlbumTags tags, String type, AppLocalizations t) async {
    if (!busy) {
      busy = true;

      // Get the direct download links for each track in the album.
      List<String> directs = [];

      etaTime.value = t.download_gathering;

      for (var i = 0; i < tags.trackURL.length; i++) {
        directs.add(await getDirectDownloadLink(baseUrl + tags.trackURL[i], type));
        currentDownload.value++;
      }

      currentDownload.value = 0;

      // Create a list of tasks to download each track in the album.
      ...
      for (var i = 0; i < directs.length; i++) {
        ...
        var task = downloadFile(directs[i]).then((value) {
          currentIndex++;
          return saveFile(value, "${tags.tracks[i]}.$type", tags.albumName);
        });
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Center(
              child: Padding(
                  ...
                  child: Marquee(child: Text(t.downloadingAlbum(tags.albumName), style: const TextStyle(fontSize: 25), textAlign: TextAlign.center)))),
          Center(
            child: Padding(
  ...
  Widget build(BuildContext context) {
    var t = AppLocalizations.of(context)!;
    downloadAlbum(tags, type, t).then((value) {
      if (value) {
